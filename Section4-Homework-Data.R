...
FTA_G <- round(Free_Throw_Attempts / Games, 2)
FTA_G

my_plot <- function(FTA_G){
  matplot(t(FTA_G,,drop = F), type = "b", pch = 15:18, col = c(1:4, 6))
  legend("bottomleft", inset = 0.0001, legend = Players, pch = 15:18, col = c(1:4, 6), horiz = F)}


my_plot <- function(z, who 1:10) {
  matplot(t(z[who,,drop = F]), type = "b", pch = 15:18, col = c(1:4, 6))
  legend("bottomleft", inset = 0.01, legend = Players[who], pch = 15:18, col = c(1:4, 6), horiz = F)
}


Accuracy_FT <- round(Free_Throws / Free_Throw_Attempts * 100, 2)
Accuracy_FT


matplot(t(FTA_G), type = "b", pch = 15:18, col = c(1:4, 6), main = "Basketball Player Analysis")
legend("bottomleft", inset = 0.0001, legend = Players, pch = 15:18, col = c(1:4, 6), horiz = F)
?matplot
Points_only_Fieldgoals <- Points - Free_Throws
Points_only_Fieldgoals

two_or_three <- round(Points_only_Fieldgoals / FieldGoals, 2)
two_or_three
